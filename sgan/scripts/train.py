...
# Dataset options
parser.add_argument("--datapath", default="sgan/data", type=str)
parser.add_argument("--testsize", default=0.4, type=float)
...
def main(args):
    ...
    train_dset, train_loader = data_loader(args, mode="train")
    logger.info("Initializing val dataset")
    _, val_loader = data_loader(args, mode="val")
    ...
    plot_dg_losses(
        ...
        checkpoint["metrics_train"]["g_l2_loss_rel"],
        filename="dg_losses",
    )
...
def plot_dg_losses(
    ...
    fig.write_image(os.path.join(image_dir, f"{filename}.png"))


def plot_first_frame(
    traj_real: np.ndarray,
    traj_fake: np.ndarray,
    filename: str,
    image_dir: str = "images",
):
    """plot read and fake timeseries
    Args:
        traj_real (np.ndarray): real (#symbols, #frames)
        traj_fake (np.ndarray): fake (#symbols, #frames)
    """
    os.makedirs(image_dir, exist_ok=True)
    n_frames = traj_real.shape[1]
    # Create traces
...
def check_accuracy(
    ...
    with torch.no_grad():
        ...
        for batch in loader:
            ...
            traj_fake = torch.cat([obs_traj, pred_traj_fake], dim=0)
            traj_fake_rel = torch.cat([obs_traj_rel, pred_traj_fake_rel], dim=0)
            first_start, first_end = seq_start_end[0]
            if plot:
                plot_first_frame(
                    traj_real.permute(1, 0, 2)[first_start:first_end, :, 0]
                    .cpu()
                    .numpy(),
                    traj_fake.permute(1, 0, 2)[first_start:first_end, :, 0]
                    .cpu()
                    .numpy(),
